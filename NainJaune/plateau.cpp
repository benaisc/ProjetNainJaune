...
//nota bene : revoir cette methode ainsi que son implémentation dans le main
//je l'ai laissee pour que vous voyez puissiez voir la direction prise
void Plateau::enterScores()
{
  ...
  if(slog.is_open())
  {
    cout<<"_Score existe, la fonction n'est pas finie dans ce cas_"<<endl;
    /*
  ...
  */
  ...
  else
  {
    ...
    cout<<"Les flux de clog sont maintenant dirigés ici d'où la surcharge"<<endl;

    size_t j=1;
    cout<<"On retire les bots"<<endl;
    ...
    if(nbJoueurs>0)
    {
      ...
      for(j=0;j<=nbJoueurs;j++){tabJoueurs[j].afficherJoueur();}
      cout<<"On l'ecrit dans le fichier"<<endl;
      streambuf *newbuf, *backup;
      backup=clog.rdbuf();
      newbuf=slog.rdbuf();
      clog.rdbuf(newbuf);
